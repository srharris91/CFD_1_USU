...
fig=plt.figure(figsize=(3,3))
ax1=fig.add_subplot(111)
ax1.contourf(x,y,phi,20,cmap=plt.cm.gray,origin='lower')
ax1.set_xlabel(r'$x$')
ax1.set_ylabel(r'$y$')
im = plt.imshow(phi, interpolation='none', extent=(0,1,0,1), cmap=plt.cm.gray, norm=cm.colors.Normalize(vmax=abs(phi).max(), vmin=abs(phi).min()))
plt.colorbar(im)
# plt.show()
# plt.legend(loc='best',frameon=False,numpoints=1)
fig.savefig('output/phi.pdf',bbox_inches='tight')
