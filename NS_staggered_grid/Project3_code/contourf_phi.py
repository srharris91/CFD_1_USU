...
ax1=fig.add_subplot(511,aspect='equal')
ax1.contourf(xu,y,u,20,cmap=plt.cm.jet,origin='lower')
ax1.set_xlabel(r'$x$')
ax1.set_ylabel(r'$y$')
ax1.set_title(r'$u$')
im = plt.imshow(u, interpolation='none', extent=(0,10,0,1), cmap=plt.cm.jet, norm=cm.colors.Normalize(vmax=(u).max(), vmin=(u).min()))
plt.colorbar(im)
# plt.show()
...
#fig=plt.figure(figsize=(3,3))
ax1=fig.add_subplot(512,aspect='equal')
ax1.contourf(x,yv,v,20,cmap=plt.cm.jet,origin='lower')
ax1.set_xlabel(r'$x$')
ax1.set_ylabel(r'$y$')
ax1.set_title(r'$v$')
im = plt.imshow(v, interpolation='none', extent=(0,10,0,1), cmap=plt.cm.jet, norm=cm.colors.Normalize(vmax=(v).max(), vmin=(v).min()))
plt.colorbar(im)
# plt.show()
# plt.legend(loc='best',frameon=False,numpoints=1)
...
ax1=fig.add_subplot(513,aspect='equal')
#ax1.contourf(x,y,p,20,cmap=plt.cm.gray,origin='lower')
ax1.contourf(x,y,p,20,cmap=plt.cm.jet,origin='lower')
ax1.set_xlabel(r'$x$')
ax1.set_ylabel(r'$y$')
ax1.set_title(r'$P$')
im = plt.imshow(p, interpolation='none', extent=(0,10,0,1), cmap=plt.cm.jet, norm=cm.colors.Normalize(vmax=(p).max(), vmin=(p).min()))
plt.colorbar(im)

ax1=fig.add_subplot(514,aspect='equal')
#ax1.contourf(x,y,p,20,cmap=plt.cm.gray,origin='lower')
...
ax1=fig.add_subplot(515,aspect='equal')
#ax1.contourf(x,y,p,20,cmap=plt.cm.gray,origin='lower')
ax1.plot(u_spot[:,1],u_spot[:,0],'ko-')
